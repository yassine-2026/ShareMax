...
const optimizerUploadMiddleware = `
...
// Insert optimizerUploadMiddleware after upload definition
code = code.replace(
  /const upload = multer\(\{[\s\S]*?\}\);/, 
  (match) => match + '\n' + optimizerUploadMiddleware
);

// Insert optimizerRoutes before Vite middleware / startServer block
code = code.replace(
...
console.log('Appended optimizer routes');
